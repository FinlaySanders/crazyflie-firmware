...
Arena* make_allocator(size_t total_size) {
    void* buffer = pvPortCalloc(1, total_size + sizeof(Arena));
...
void _gelu(const float* input, float* output, int size) {
    for (int i = 0; i < size; i++) {
        ...
        output[i] = 0.5f * input[i] *
                    (1.0f + tanhf(0.66285265f * (input[i] + 0.044715f*input[i]*input[i]*input[i])));
    }
}
...
void _lstm(const float* input, float* state_h, float* state_c, const float* weights_input,
    ...
    for (int b=0; b<batch_size; b++) {
        ...
        for (int i=2*hidden_size; i<3*hidden_size; i++) {
            int buf_adr = b_offset + i;
            buffer[buf_adr] = tanhf(buffer[buf_adr]);
        }
    ...
    for (int b=0; b<batch_size; b++) {
        ...
        for (int i=0; i<hidden_size; i++) {
            ...
            state_h[hidden_offset + i] = (
                buffer[b_offset + 3*hidden_size + i] * tanhf(state_c[hidden_offset + i])
            );
        }
    }
}
...
Linear* make_linear(Weights* weights, int batch_size, int input_dim, int output_dim) {
    ...
    Linear* layer = pvPortCalloc(1, sizeof(Linear) + buffer_size);
    *layer = (Linear){
...
ReLU* make_relu(int batch_size, int input_dim) {
    size_t buffer_size = (size_t)batch_size*(size_t)input_dim*sizeof(float);
    ReLU* layer = pvPortCalloc(1, sizeof(ReLU) + buffer_size);
    *layer = (ReLU){
        .output = (float*)(layer + 1),
        .batch_size = batch_size,
        .input_dim = input_dim,
...
GELU* make_gelu(int batch_size, int input_dim) {
    size_t buffer_size = (size_t)batch_size*(size_t)input_dim*sizeof(float);
    GELU* layer = pvPortCalloc(1, sizeof(GELU) + buffer_size);
    *layer = (GELU){
        .output = (float*)(layer + 1),
        .batch_size = batch_size,
        .input_dim = input_dim,
...
MaxDim1* make_max_dim1(int batch_size, int seq_len, int feature_dim) {
    size_t buffer_size = (size_t)batch_size*(size_t)feature_dim*sizeof(float);
    MaxDim1* layer = pvPortCalloc(1, sizeof(MaxDim1) + buffer_size);
    *layer = (MaxDim1){
        .output = (float*)(layer + 1),
        .batch_size = batch_size,
        .seq_len = seq_len,
...
Conv2D* make_conv2d(Weights* weights, int batch_size, int in_width, int in_height,
    ...
    int h_out = (in_height - kernel_size)/stride + 1;
    int w_out = (in_width - kernel_size)/stride + 1;
    size_t buffer_size = (size_t)batch_size*(size_t)out_channels*(size_t)h_out*(size_t)w_out*sizeof(float);
    int num_weights = out_channels*in_channels*kernel_size*kernel_size;
    Conv2D* layer = pvPortCalloc(1, sizeof(Conv2D) + buffer_size);
    *layer = (Conv2D){
        .output = (float*)(layer + 1),
...
Conv3D* make_conv3d(Weights* weights, int batch_size, int in_width, int in_height, int in_depth,
    ...
    int h_out = (in_height - kernel_size)/stride + 1;
    int w_out = (in_width - kernel_size)/stride + 1;
    size_t buffer_size = (size_t)batch_size*(size_t)out_channels*(size_t)d_out*(size_t)h_out*(size_t)w_out*sizeof(float);
    int num_weights = out_channels*in_channels*kernel_size*kernel_size*kernel_size;
    Conv3D* layer = pvPortCalloc(1, sizeof(Conv3D) + buffer_size);
    *layer = (Conv3D){
        .output = (float*)(layer + 1),
        .weights = get_weights(weights, num_weights),
        .bias = get_weights(weights, out_channels),
...
LSTM* make_lstm(Weights* weights, int batch_size, int input_size, int hidden_size) {
    ...
    LSTM* layer = pvPortCalloc(1, sizeof(LSTM) + (size_t)6*state_size*sizeof(float));
...
Embedding* make_embedding(Weights* weights, int batch_size, int num_embeddings, int embedding_dim) {
    size_t output_size = (size_t)batch_size*(size_t)embedding_dim*sizeof(float);
    Embedding* layer = (Embedding*)pvPortCalloc(1, sizeof(Embedding) + output_size);
    *layer = (Embedding){
        .output = (float*)(layer + 1),
...
LayerNorm* make_layernorm(Weights* weights, int batch_size, int input_dim) {
    size_t output_size = (size_t)batch_size*(size_t)input_dim*sizeof(float);
    LayerNorm* layer = pvPortCalloc(1, sizeof(LayerNorm) + output_size);
    *layer = (LayerNorm){
        .output = (float*)(layer + 1),
...
OneHot* make_one_hot(int batch_size, int input_size, int num_classes) {
    size_t buffer_size = (size_t)batch_size*(size_t)input_size*(size_t)num_classes*sizeof(int);
    OneHot* layer = pvPortCalloc(1, sizeof(OneHot) + buffer_size);
    *layer = (OneHot){
        .output = (int*)(layer + 1),
        .batch_size = batch_size,
        .input_size = input_size,
...
CatDim1* make_cat_dim1(int batch_size, int x_size, int y_size) {
    size_t buffer_size = (size_t)batch_size*((size_t)x_size + (size_t)y_size)*sizeof(float);
    CatDim1* layer = pvPortCalloc(1, sizeof(CatDim1) + buffer_size);
    *layer = (CatDim1){
        .output = (float*)(layer + 1),
        .batch_size = batch_size,
        .x_size = x_size,
...
Multidiscrete* make_multidiscrete(int batch_size, const int logit_sizes[], int num_actions) {
    ...
    memcpy(layer->logit_sizes, logit_sizes, (size_t)num_actions*sizeof(int));
    return layer;
}
...
Default* make_default(Weights* weights, int num_agents, int input_dim, int hidden_dim, int action_dim) {
    Default* net = pvPortCalloc(1, sizeof(Default));
    net->num_agents = num_agents;
    net->obs = (float*)pvPortCalloc((size_t)num_agents*(size_t)input_dim, sizeof(float));
...
void free_default(Default* net) {
    vPortFree(net->obs);
    vPortFree(net->encoder);
    vPortFree(net->relu1);
    vPortFree(net->actor);
    vPortFree(net->value_fn);
    vPortFree(net->multidiscrete);
    vPortFree(net);
}
...
ConvLSTM* make_convlstm(Weights* weights, int num_agents, int input_dim,
    ...
    ConvLSTM* net = pvPortCalloc(1, sizeof(ConvLSTM));
    net->num_agents = num_agents;
    net->obs = pvPortCalloc((size_t)num_agents*(size_t)input_dim*(size_t)input_dim*(size_t)input_channels, sizeof(float));
